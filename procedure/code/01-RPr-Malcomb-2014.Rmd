...
cluster_ta <- dhsclusters %>% 
  st_join(ta_v) %>% 
  st_drop_geometry() %>% 
  dplyr::select(DHSCLUST, ta_id = ID_2, urban_rural = URBAN_RURA)
...
ta_v <- left_join(
  ta_v,
  dplyr::select(ta_capacity_t, ta_id, rpac = capacity_avg),
  by = c("ID_2" = "ta_id")
)
...
ta_stats <- ta_v %>% 
  st_drop_geometry %>% 
  dplyr::select(rpac_unscaled, rpac) %>% 
  stat.desc() %>%
  mutate_if(is.numeric, round, digits=2)
...
# join to ta_v
ta_fig4_t <- ta_fig4 %>% st_drop_geometry() %>% dplyr::select(ID_2, orac)
...
```{r vulnerability-difference-map-tmap}
# Set tmap mode
tmap_mode("view")

# Create tmap based on ggplot above
vuln_diff_tmap <- tm_shape(vulnerability_r["diffv"]) + tm_raster(
    palette = "PuOr",
    style = "cont",
    breaks = c(-range,0,range),
    labels = c(paste0("-",range), "0", paste0(range)),
...
```{r}
# Output descriptive statistics of difference band
vulnerability_r["diffv"]

# Convert band to raster object to facilitate histogram
vuln_raster <- vulnerability_r %>% as("Raster")

diff_raster <- vuln_raster$diffv

diff_values <- values(diff_raster) %>% na.omit()

# Plot a histogram of the difference values
hist(diff_values,
     main = "Distribution of difference values",
     xlab = "Vulnerability: Reproduction - Original", ylab = "Frequency",
     col = "darkgray",
     freq=T)
```
